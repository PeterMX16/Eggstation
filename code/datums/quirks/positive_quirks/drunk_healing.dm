...
/datum/quirk/drunkhealing/process(seconds_per_tick)
	switch(quirk_holder.get_drunk_amount())
		if (6 to 40)
			quirk_holder.adjustBruteLoss(-0.1 * seconds_per_tick, FALSE)
			quirk_holder.adjustFireLoss(-0.05 * seconds_per_tick)
		if (41 to 60)
			quirk_holder.adjustBruteLoss(-0.4 * seconds_per_tick, FALSE)
			quirk_holder.adjustFireLoss(-0.2 * seconds_per_tick)
		if (61 to INFINITY)
			quirk_holder.adjustBruteLoss(-0.8 * seconds_per_tick, FALSE)
			quirk_holder.adjustFireLoss(-0.4 * seconds_per_tick)
=======
	quirk_flags = QUIRK_HUMAN_ONLY|QUIRK_PROCESSES
	mail_goodies = list(/obj/effect/spawner/random/food_or_drink/booze)
